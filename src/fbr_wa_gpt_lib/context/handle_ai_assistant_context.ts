...
export async function handle_message_user_context_assistant(client: Client, message: WAWebJS.Message, chat: WAWebJS.Chat, userContext: db_user): Promise<void> {
    ...
    if (!userContext.active_chat.assistant_runnable) {
        ...
        userContext.active_chat.assistant_runnable = new OpenAIAssistantWrapperV2({
            ...
            functions: new Map(
                [
                    ['clear_init_reset_new_chat', async () => {
                        // clear the chat and start a new one, this is useful for when the user wants to start a new chat
                        userContext.active_chat.assistant_runnable = undefined;
                        await updateUser(userContext.id, userContext);
                        console.log('Chat cleared, starting new chat...');
                        return 'Chat cleared, you can start a new conversation now.';
                    }],
                ]
            ),
        });
